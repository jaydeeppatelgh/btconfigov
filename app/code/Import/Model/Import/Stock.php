 ...
 */
class Stock extends ImportAbstract implements ImportInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->addMessage(__('Start stock import'));
        $import = $this->rapidFlowHelper->run($this->borntechiesHelper->getStockImportProfile());

        $this->addMessage(__('Import stock to update'));
        ...
        if ($import->getCurrentActivity() != __('Done')) {
            throw new LocalizedException(__('Error during stock import'));
        }
    }
}
